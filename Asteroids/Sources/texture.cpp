...
/*static*/
GLuint TextureBox::loadTexture( const std::string & fName ) {
    int width, height;
    unsigned char * data = TextureBox::loadPixels(fName, width, height);
...
unsigned char *TextureBox::loadPixels(const std::string &fName, int & width, int & height) {
    int bytesPerPix;
...
GLuint TextureBox::loadCubeMap(const std::string &baseName) {
    GLuint texID;
    ...
    GLubyte * data = TextureBox::loadPixels(texName.c_str(), w, h);
